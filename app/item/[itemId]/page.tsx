...
export default async function ItemPage({ params }: ItemPageProps) {
  ...
  // Debug log for params.itemId
  // eslint-disable-next-line no-console
  console.log("[ItemPage] params.itemId:", params.itemId);

  // Accept any non-empty string as a valid item ID
  if (!params.itemId || typeof params.itemId !== "string" || params.itemId.trim() === "") {
    // Show the actual value of params.itemId for debugging
    return (
      <div className="min-h-screen bg-[#F2F2F2] flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Invalid Item ID</h1>
          <p className="text-[#666666] mb-4">The item ID provided is not valid: <span style={{color: 'red', fontWeight: 'bold'}}>{JSON.stringify(params.itemId)}</span></p>
          <p className="text-[#666666] mb-4">(Debug: typeof params.itemId = {typeof params.itemId})</p>
  ...
  // Try both UUID and TEXT lookup for compatibility
    let itemData = null;
    let itemError = null;
  if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(params.itemId)) {
