...
export default async function ItemPage({ params }: ItemPageProps) {
  ...
  console.log("[ItemPage] id:", id);

  // Accept any non-empty string as a valid item ID
  if (!id || typeof id !== "string" || id.trim() === "") {
    // Show the actual value of id for debugging
    return (
      <div className="min-h-screen bg-[#F2F2F2] flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Invalid Item ID</h1>
          <p className="text-[#666666] mb-4">The item ID provided is not valid: <span style={{color: 'red', fontWeight: 'bold'}}>{JSON.stringify(id)}</span></p>
          <p className="text-[#666666] mb-4">(Debug: typeof id = {typeof id})</p>
